...
> Last updated: **2026-07-04 12:49 WIB**
...
Halaman-halaman ini harus selesai terlebih dahulu karena saling bergantung.
Tidak bisa generate unit kalau MO belum bisa diubah statusnya ke `DONE`.
...
- `[ ]` Layout full-width (tidak containered)
...
- `[ ]` List semua unit dengan urutan FEFO (First Expired First Out)
...
- `[x]` Fix `ProductionUnitResponse` (hari_tersisa, is_expired, is_expiring_soon) — 2026-07-04
- `[x]` Fix `ExpiryAlertResponse` field mismatch — 2026-07-04
- `[x]` Fix `ScanVoidRequest.alasan` — 2026-07-04
- `[ ]` Cek endpoint `GET /menu/` — pastikan response include `resep_list` + `bahan_list` per resep
- `[ ]` Cek endpoint `GET /manufacturing-orders/{id}` — pastikan include `bahan_baku_lines`
- `[ ]` Cek endpoint `PATCH /manufacturing-orders/{id}/status` — ada atau perlu dibuat?
