...
class AbsensiRepository:
    ...
    def get_by_user_tanggal(self, user_id: int, tanggal: date) -> Optional[Absensi]:
        return self.db.scalar(
            select(Absensi).where(
                and_(Absensi.user_id == user_id, Absensi.tanggal == tanggal)
            )
        )
    ...
    def list_by_tanggal(self, tanggal: date) -> list[Absensi]:
        return list(
            self.db.scalars(
                select(Absensi).where(Absensi.tanggal == tanggal)
                .order_by(Absensi.user_id)
            )
        )
    ...
    def create(self, data: AbsensiCreate, dicatat_oleh: int) -> Absensi:
        obj = Absensi(
