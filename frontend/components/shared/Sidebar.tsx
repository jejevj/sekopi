...
export function Sidebar() {
  ...
  return (
    <View style={{ width: 220, height: '100%', backgroundColor: '#111', borderRightWidth: 1, borderRightColor: '#1f1f1f', padding: 16, display: 'flex', flexDirection: 'column' }}>
      <Text style={{ color: 'white', fontSize: 18, fontWeight: '700', marginBottom: 24 }}>☕ SekoPi</Text>

      <View style={{ flex: 1 }}>
        {menus.map((item) => {
          const active = pathname.startsWith(item.href.replace('/(admin)', '').replace('/(produksi)', '').replace('/(driver)', '').replace('/(inventori)', '').replace('/(shareholder)', ''));
          return (
            <Link
              key={item.href}
