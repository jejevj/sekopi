...
export default function LoadingPage() {
  ...
  async function fetchAll() {
    ...
    } catch { showToast('Gagal memuat data', false); }
  ...
  async function createOrder() {
    ...
    try {
      ...
      fetchAll();
    } catch { showToast('Gagal membuat loading order', false); }
  ...
  async function updateStatus(order: LoadingOrder, newStatus: string) {
    try {
      ...
      fetchAll();
    } catch (e: any) { showToast(e?.response?.data?.detail ?? 'Gagal ubah status', false); }
  }
  ...
  async function doScan() {
    ...
    try {
      ...
      setBarcode('');
      fetchAll();
    } catch (e: any) { setScanMsg({ ok: false, msg: e?.response?.data?.detail ?? 'Gagal scan' }); }
  ...
  // ─── glass token (sesuai global.css) ───────────────────────────────────────
  const glass      = { background: 'rgba(255,255,255,0.04)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', borderRadius: 16 } as const;
  const glassCard  = { background: 'rgba(255,255,255,0.06)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)',  borderRadius: 14 } as const;
  ...
  const btnGhost   = { backgroundColor: 'rgba(255,255,255,0.06)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.12)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0f1117' }}>
      ...
      {/* Header */}
      <View style={{ borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.08)', paddingHorizontal: 24, paddingVertical: 16,
      ...
      {loading ? (
        ...
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', gap: 12 }}>
          <PackageCheck size={48} color="rgba(255,255,255,0.1)" />
          <Text style={{ color: 'rgba(255,255,255,0.3)', fontSize: 15 }}>Belum ada loading order</Text>
        </View>
      ) : (
            ...
            return (
              <View key={order.id} style={{ ...glassCard, marginBottom: 12, overflow: 'hidden' }}>
                ...
                {order.items.length > 0 && (
                  <View style={{ borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.06)', paddingHorizontal: 16, paddingVertical: 10 }}>
                    {order.items.map(item => (
                      <View key={item.id} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 3 }}>
                        <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', fontFamily: 'monospace' }}>{item.barcode_snapshot}</Text>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                          <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)' }}>Rp {Number(item.harga_modal_snapshot).toLocaleString('id')}</Text>
                          {order.status === 'draft' && (
      ...
      <Modal visible={showCreate} transparent animationType="fade">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <View style={{ ...glass, width: '100%', maxWidth: 440, padding: 24, backgroundColor: '#161b27' }}>
            <Text style={{ fontWeight: '700', fontSize: 16, color: 'white', marginBottom: 16 }}>Buat Loading Order</Text>

            <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.8 }}>Gerobak</Text>
              ...
              {gerobaks.map(g => (
                <Pressable key={g.id} onPress={() => setForm(f => ({ ...f, gerobak_id: String(g.id) }))}
                  style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 10,
                    backgroundColor: form.gerobak_id === String(g.id) ? 'rgba(244,68,68,0.12)' : 'transparent' }}>
                  <Text style={{ fontSize: 13, color: form.gerobak_id === String(g.id) ? '#f87171' : 'rgba(255,255,255,0.7)' }}>
                    {g.nama} <Text style={{ color: 'rgba(255,255,255,0.3)' }}>({g.kode})</Text>
                  </Text>
            ...
            <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.8 }}>Driver</Text>
            <View style={{ borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', borderRadius: 10, marginBottom: 14, overflow: 'hidden' }}>
              {users.map(u => (
                <Pressable key={u.id} onPress={() => setForm(f => ({ ...f, driver_id: String(u.id) }))}
                  style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 10,
                    backgroundColor: form.driver_id === String(u.id) ? 'rgba(244,68,68,0.12)' : 'transparent' }}>
                  <Text style={{ fontSize: 13, color: form.driver_id === String(u.id) ? '#f87171' : 'rgba(255,255,255,0.7)' }}>
                    {u.full_name} <Text style={{ color: 'rgba(255,255,255,0.3)' }}>({u.role})</Text>
                  </Text>
            ...
            <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.8 }}>Catatan</Text>
            <TextInput value={form.catatan} onChangeText={t => setForm(f => ({ ...f, catatan: t }))}
              ...
              style={{ ...inputStyle, textAlignVertical: 'top', marginBottom: 20 }} />

            <View style={{ flexDirection: 'row', gap: 10, justifyContent: 'flex-end' }}>
              <Pressable onPress={() => setShowCreate(false)}
                style={{ ...btnGhost, paddingHorizontal: 16, paddingVertical: 9 }}>
                <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13 }}>Batal</Text>
              </Pressable>
              <Pressable onPress={createOrder} disabled={saving || !form.gerobak_id || !form.driver_id}
                style={{ ...btnPrimary, paddingVertical: 9, opacity: (!form.gerobak_id || !form.driver_id) ? 0.4 : 1 }}>
      ...
      <Modal visible={!!scanTarget} transparent animationType="slide">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#161b27', borderTopLeftRadius: 24, borderTopRightRadius: 24,
            ...
            <Text style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginBottom: 14 }}>
              {scanTarget?.nomor_loading} —{' '}
              <Text style={{ color: 'rgba(255,255,255,0.6)' }}>{scanTarget?.items.length ?? 0} unit terscan</Text>
            </Text>
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
              ...
              <Pressable onPress={doScan} disabled={scanning || !barcode.trim()}
                style={{ ...btnPrimary, paddingHorizontal: 16, opacity: !barcode.trim() ? 0.4 : 1 }}>
                {scanning
            ...
            {scanMsg && (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, padding: 10, borderRadius: 10,
                backgroundColor: scanMsg.ok ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)',
                borderWidth: 1, borderColor: scanMsg.ok ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)' }}>
                {scanMsg.ok
