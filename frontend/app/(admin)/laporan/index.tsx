...
// ─── Fetch semua unit SOLD via auto-paginate (max 500/halaman) ────────────────────
async function fetchAllSoldUnits(): Promise<SoldUnit[]> {
  const PER_PAGE = 500;
  let page = 1;
  const collected: SoldUnit[] = [];

  while (true) {
    const res = await api.get(`/production-units/?status=sold&page=${page}&per_page=${PER_PAGE}`);
    const data = res.data;
    const items: SoldUnit[] = Array.isArray(data) ? data : (data.items ?? []);
    collected.push(...items);

    // Berhenti kalau halaman ini tidak penuh (artinya sudah halaman terakhir)
...
export default function LaporanPenjualanPage() {
  ...
  // Fetch hanya unit SOLD, auto-paginate
  const { data: allUnits = [], isLoading, refetch } = useQuery<SoldUnit[]>({
    queryKey: ['sold-units-laporan'],
    queryFn: fetchAllSoldUnits,
    staleTime: 30_000,
  });

  const filtered = useMemo(() => {
    let list = allUnits;
    if (dateFrom) list = list.filter(u => u.sold_at && u.sold_at.slice(0, 10) >= dateFrom);
    if (dateTo)   list = list.filter(u => u.sold_at && u.sold_at.slice(0, 10) <= dateTo);
    if (search.trim()) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: '#0a0a0a' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            ...
            <p style={{ color: '#555', fontSize: 13, margin: '4px 0 0' }}>
              {isLoading ? 'Memuat...' : `${totalUnit} unit terjual`}
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
          <StatCard icon={<ShoppingBag size={14} color="#4ade80" />} label="Unit Terjual"
            value={isLoading ? '...' : totalUnit.toString()} color="#4ade80" sub="unit dengan status SOLD" />
          <StatCard icon={<TrendingUp size={14} color="#60a5fa" />} label="Total Omzet"
            value={isLoading ? '...' : rp(totalOmzet)} color="#60a5fa"
            sub={isLoading ? '' : `Rata-rata ${rp(Math.round(avgJual))} / unit`} />
          <StatCard icon={<Banknote size={14} color="#a3e635" />} label="Laba Kotor"
            value={isLoading ? '...' : rp(totalProfit)}
            color={totalProfit >= 0 ? '#a3e635' : '#f87171'}
            sub={isLoading ? '' : `Modal: ${rp(totalModal)}`} />
          <StatCard icon={<BarChart2 size={14} color="#f59e0b" />} label="Margin"
            value={isLoading ? '...' : totalOmzet > 0 ? ((totalProfit / totalOmzet) * 100).toFixed(1) + '%' : '—'}
            color="#f59e0b" sub="Laba / Omzet" />
        </div>

        {isLoading ? (
          <div style={{ textAlign: 'center', padding: 60, color: '#555' }}>
            <RefreshCw size={20} color="#333" style={{ marginBottom: 12, animation: 'spin 1s linear infinite' }} />
            <p style={{ margin: 0, fontSize: 13 }}>Memuat data penjualan...</p>
          </div>
        ) : (
          <>
            {/* ── Grid: per produk + export ── */}
                ...
                {byProduk.length === 0 ? (
                  ...
                  <div key={p.nama} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <p style={{ color: p.omzet - p.modal >= 0 ? '#a3e635' : '#f87171', fontSize: 11, margin: '2px 0 0' }}>laba {rp(p.omzet - p.modal)}</p>
                    </div>
                  </div>
                ))}
              </div>
                      ...
                      {filtered.map((u, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ padding: '9px 10px' }}>
                              {u.loading_order_id
                                ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#f59e0b' }}><Truck size={10} color="#f59e0b" /> #{u.loading_order_id}</span>
