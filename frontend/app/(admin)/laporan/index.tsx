...
// Konversi datetime-local value ("YYYY-MM-DDTHH:mm") ke ISO WIB untuk dikirim ke API
function localInputToWib(val: string): string {
  // val sudah dalam konteks WIB karena user input berdasarkan waktu lokal WIB
  // Kirim apa adanya, backend akan parse sebagai WIB
  return val; // format: "2026-07-04T19:30"
}

// Preset helper — return datetime-local string ("YYYY-MM-DDTHH:mm") dalam WIB
function nowWib(): Date {
  return new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Jakarta' }));
}
function isoLocalWib(d: Date): string {
  const pad = (n: number) => String(n).padStart(2, '0');
  return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`;
}
function startOfDayWib(d: Date): string {
  const pad = (n: number) => String(n).padStart(2, '0');
  return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T00:00`;
}
function endOfDayWib(d: Date): string {
  const pad = (n: number) => String(n).padStart(2, '0');
...
const PRESETS = [
  { label: 'Hari Ini',  from: startOfDayWib(todayWib),            to: endOfDayWib(todayWib) },
  { label: '7 Hari',   from: startOfDayWib(addDays(todayWib,-6)), to: endOfDayWib(todayWib) },
  { label: '30 Hari',  from: startOfDayWib(addDays(todayWib,-29)),to: endOfDayWib(todayWib) },
  { label: 'Semua',    from: '',                                   to: '' },
];

// ─── Fetch all penjualan dengan pagination ───────────────────────────────────
async function fetchAllPenjualan(dari: string, sampai: string): Promise<PenjualanItem[]> {
  const PER_PAGE = 500;
  let page = 1;
...
export default function LaporanPenjualanPage() {
  const [dateFrom,      setDateFrom]      = useState(startOfDayWib(todayWib));
  const [dateTo,        setDateTo]        = useState(endOfDayWib(todayWib));
  ...
  const { data: allItems = [], isLoading, refetch } = useQuery<PenjualanItem[]>({
    ...
    queryFn: () => fetchAllPenjualan(
      localInputToWib(dateFrom),
      localInputToWib(dateTo),
    ),
    staleTime: 30_000,
  });

  // Dropdown options dari data yang sudah difetch
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => new Date(b.sold_at).getTime() - new Date(a.sold_at).getTime());
  ...
  const byGerobak = useMemo(() => {
    ...
    for (const u of filtered) {
      const key = String(u.gerobak_id ?? 'x');
      const e = map.get(key) ?? { nama: u.gerobak_nama ?? 'Tanpa Gerobak', kode: u.gerobak_kode ?? '-', grup: u.grup_nama ?? '-', qty: 0, omzet: 0 };
      e.qty++; e.omzet += u.harga;
      map.set(key, e);
    }
  ...
  const dtInpStyle: React.CSSProperties = { ...inp, width: 180, colorScheme: 'dark' };
  const selStyle:   React.CSSProperties = { ...inp, width: 'auto', minWidth: 140 };
        ...
        <div style={{ ...card, marginBottom: 16, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <span style={{ color: '#444', fontSize: 10 }}>DARI (WIB)</span>
              <input
                type="datetime-local"
                value={dateFrom}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <span style={{ color: '#444', fontSize: 10 }}>SAMPAI (WIB)</span>
              <input
                type="datetime-local"
                value={dateTo}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <select value={filterGerobak} onChange={e => setFilterGerobak(e.target.value)} style={selStyle}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <select value={filterGrup} onChange={e => setFilterGrup(e.target.value)} style={selStyle}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                    ...
                    <div key={g.kode} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      <div>
                        ...
                        <p style={{ color: '#555', fontSize: 11, margin: '2px 0 0' }}>
                          {g.kode !== '-' ? `[${g.kode}] · ` : ''}<span style={{ color: '#7c3aed' }}>{g.grup}</span> · {g.qty} unit
                        </p>
                      </div>
              ...
              <div style={card}>
                ...
                {byProduk.length === 0
                  ? <p style={{ color: '#444', fontSize: 13 }}>Tidak ada data</p>
                  : byProduk.map(p => (
                    <div key={p.nama} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      <div>
                        <p style={{ color: 'white', fontSize: 13, margin: 0 }}>{p.nama}</p>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                        {['Waktu Jual (WIB)', 'Barcode', 'Produk', 'Harga Jual', 'Gerobak', 'Driver / Kasir', 'Grup Saham'].map(h => (
                      ...
                      {filtered.map((u, i) => (
                        ...
                        >
                          <td style={{ padding: '9px 10px', whiteSpace: 'nowrap' }}>
                            <p style={{ color: 'white', margin: 0, fontSize: 12 }}>{fmtDate(u.sold_at)}</p>
                            <p style={{ color: '#f59e0b', margin: 0, fontSize: 11, fontWeight: 600 }}>{fmtTime(u.sold_at)}</p>
                          </td>
                          ...
                          <td style={{ padding: '9px 10px' }}>
                            {u.kasir_nama ? <Badge text={u.kasir_nama} color="#f59e0b" /> : <span style={{ color: '#333' }}>—</span>}
                          </td>
                          <td style={{ padding: '9px 10px' }}>
                            {u.grup_nama ? <Badge text={u.grup_nama} color="#a78bfa" /> : <span style={{ color: '#333' }}>—</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
...
function CsvExportBlock({ items, dateFrom, dateTo }: { items: PenjualanItem[]; dateFrom: string; dateTo: string }) {
  const handleExport = () => {
    const header = ['Tanggal', 'Jam (WIB)', 'Barcode', 'Produk', 'Harga Jual', 'Gerobak', 'Kode', 'Lokasi', 'Driver/Kasir', 'Grup Saham'].join(',');
    ...
    a.href     = url;
    a.download = `laporan-penjualan-${dateFrom.replace('T','_').slice(0,16)}-sd-${dateTo.replace('T','_').slice(0,16)}.csv`;
    a.click();
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
      <p style={{ color: '#555', fontSize: 12, margin: 0 }}>
        Kolom: Tanggal, Jam (WIB), Barcode, Produk, Harga, Gerobak, Kode, Lokasi, Driver, Grup Saham
      </p>
      <button onClick={handleExport} disabled={items.length === 0} style={{
        padding: '9px 20px', borderRadius: 10, fontSize: 13, fontWeight: 700,
        cursor: items.length === 0 ? 'not-allowed' : 'pointer', border: '1px solid',
